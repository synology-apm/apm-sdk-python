...
- When an API field name differs from the SDK dataclass field name, perform the conversion inside the collection parser (`_parse_*` functions) without changing the SDK's public interface. Magic values (such as `"-1"` / `""`) on `changeDataSize`/`dedupedDataSize`/`transferredSize`/`transferredDataSize` are always converted to `None` via `_parse_data_sizes()` / `_parse_int_or_none()`; `durationTime` (restore) gets an equivalent inline `""`/`None` check in `_parse_restore_activity()` rather than routing through `_parse_int_or_none()`, since its downstream handling (keep only if positive) differs from that helper's sentinel-equality logic.
