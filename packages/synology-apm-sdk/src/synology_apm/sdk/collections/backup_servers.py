...
async def _build_tiering_plan_cache(
    ...
    for s in servers_raw:
        uid = (s.get("spec", {}).get("tieringPlanRef") or {}).get("uid", "")
...
def _parse_backup_server(raw: dict[str, Any], tiering_cache: _TieringPlanCache) -> BackupServer:
    ...
    if spec_sync == "JOINING":
        ...
        server_status = _SERVER_STATUS_MAP.get(status.get("status", ""), ServerStatus.DISCONNECTED)
    system_version = None if is_nas else status.get("firmwareVer", "")

    upgrade: dict[str, Any] = status.get("upgrade", {})
    upgrade_status = upgrade.get("upgradeStatus", "")
    ...
    if is_nas:
        nas_list: list[dict[str, Any]] = status.get("nasStorage", [])
        if nas_list:
            storage_total = sum(int(s.get("totalBytes", 0)) for s in nas_list)
            storage_used = sum(int(s.get("usedBytes", 0)) for s in nas_list)
        else:
            storage_total = None
            storage_used = None
    else:
        dp: dict[str, Any] = status.get("dpStorage", {})
        if dp:
            storage_total = int(dp.get("totalBytes", 0))
            storage_used = int(dp.get("backupBytes", 0)) + int(dp.get("systemBytes", 0))
        else:
            storage_total = None
            storage_used = None
    ...
    if stat:
        logical_bytes  = int(stat.get("transferBytes", 0))
        physical_bytes = int(stat.get("usageBytes", 0))
    else:
        logical_bytes  = None
        physical_bytes = None

    plan_uid = (spec.get("tieringPlanRef") or {}).get("uid", "")
    ...
    tiering_status = _parse_tiering_status(raw.get("tieringInfo", {}))

    return BackupServer(
        backup_server_id=raw.get("id", ""),
        namespace=raw.get("namespace", ""),
        server_type=server_type,
        name=status.get("hostName", ""),
        hostname=spec.get("addr", ""),
