...
def _parse_log_entries(raw_logs: list[dict[str, Any]]) -> tuple[ActivityLogEntry, ...]:
    ...
    for log in raw_logs:
        ...
        level = _LOG_LEVEL_MAP.get(log.get("level") or "", LogLevel.INFO)
        entries.append(ActivityLogEntry(timestamp=dt, level=level, message=log.get("description") or ""))
...
def _parse_restore_from_info(raw: dict[str, Any] | None) -> LocationInfo | None:
    ...
    if not raw:
        return None
    return LocationInfo(
        is_remote_storage=(raw.get("destinationType") or "APPLIANCE") != "APPLIANCE",
        identifier="",
        name=raw.get("hostname") or "",
        endpoint=raw.get("address") or "",
...
def _parse_destination_inventory(machine_info: dict[str, Any] | None) -> Hypervisor | None:
    ...
    return Hypervisor(
        hypervisor_id="",
        hostname=name,
        address=info.get("inventory_addr") or "",
        host_type=_HOST_TYPE_MAP.get(info.get("inventory_type") or "", HypervisorType.UNKNOWN),
        account="",
        description="",
        port=0,
        version="",
...
def _parse_activity_common(raw: dict[str, Any]) -> dict[str, Any]:
    ...
    status_raw: dict[str, Any] = raw.get("status") or {}

    subtype = _RAW_TO_SUBTYPE.get(spec.get("workloadType") or "", ActivityWorkloadType.UNKNOWN)
    workload_ref = spec.get("workload") or {}

    processed_success_count = processed_warning_count = processed_error_count = None
    if subtype in _ITEM_BASED_SUBTYPES:
        processed_success_count = int(status_raw.get("processedSuccessCount") or 0)
        processed_warning_count = int(status_raw.get("processedWarningCount") or 0)
        processed_error_count   = int(status_raw.get("processedErrorCount") or 0)

    return {
        "activity_id": raw.get("uid") or "",
        "workload_id": workload_ref.get("uid") or "",
        "workload_name": spec.get("workloadName") or "",
        "category": _ACTIVITY_TYPE_TO_CATEGORY[subtype],
        "workload_type": subtype,
        "namespace": raw.get("namespace") or "",
        "plan_name": spec.get("planName") or "",
        "started_at": _parse_ts_or_now(status_raw.get("startTime")),
        "finished_at": _parse_ts_optional(status_raw.get("endTime")),
        "progress": int(status_raw.get("progress") or 0),
...
def _parse_restore_activity(raw: dict[str, Any]) -> RestoreActivity:
    ...
    restore_status = status_raw.get("restoreStatus") or ""
    ...
    return RestoreActivity(
        **common,
        ...
        restore_type=(
            _RESTORE_TYPE_MAP.get(spec.get("restoreType") or "", RestoreType.UNKNOWN)
...
def _parse_backup_activity(raw: dict[str, Any]) -> BackupActivity:
    ...
    return BackupActivity(
        **common,
        execution_id=status_raw.get("executionId") or "",
