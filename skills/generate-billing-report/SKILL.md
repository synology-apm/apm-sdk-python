...
description: "Compute backup charges per plan, server, workload type, tenant, or domain from protected data volume. Use when the user asks for a billing or chargeback report."
---
...
1. Call `list_machine_workloads` (limit=500), with `is_retired=true` in a second pass to also
   capture retired (e.g. Retirement-Plan-billed) workloads. Call `list_saas_applications` first to
   enumerate tenants and domains, then call `list_m365_workloads` once per (tenant,
   `workload_type`) pair — covering all six types (exchange, onedrive, chat, sharepoint, teams,
   group) — and `list_gws_workloads` once per (domain, `workload_type`) pair — covering all five
   types (mail, calendar, contact, drive, shared_drive). Each takes a single `workload_type` and
   has no "all types" option, so call each with both `is_retired=false` and `is_retired=true`.
   Every one of these calls returns `{items, total}` — if `total` exceeds the returned item
   count, keep paging with `offset` until you've collected everything (see
   `apm-mcp-conventions`); a charge computed from a partial page silently undercounts.

2. Group workloads by the dimension the user requests:
   - **By plan**: group on `plan.name`
   - **By server**: group on `backup_server.name`
   - **By type**: group on `workload_type` (vm, pc, exchange, mail, ...)
   - **By tenant/domain**: group on `tenant_id` (M365) or `domain` (GWS)
