"""Unit tests for apm gws mail version commands: list/get/lock/unlock."""
...
def test_gws_mail_version_list_direct_mode_table() -> None:
    """gws mail version list --workload-id --namespace should list versions (table)."""
    mock_apm = make_mock_apm()
    mock_apm.gws.workloads.list_versions.return_value = ([SAMPLE_VERSION], 1)

    result = invoke_cli(mock_apm, [
        "gws", "mail", "version", "list",
        "--workload-id", WORKLOAD_ID, "--namespace", NAMESPACE,
    ], env={"COLUMNS": "300"})

    assert result.exit_code == 0, result.output
    assert "ver-gws-001" in result.output
    assert "Showing 1 of 1" in result.output

def test_gws_mail_version_list_passes_offset_to_sdk() -> None:
    """version list --offset N should pass offset=N to list_versions and show footer with correct range."""
    mock_apm = make_mock_apm()
    mock_apm.gws.workloads.list_versions.return_value = ([SAMPLE_VERSION], 26)
    ...
    assert result.exit_code == 0, result.output
    call_kwargs = mock_apm.gws.workloads.list_versions.call_args.kwargs
...
def test_gws_mail_version_list_json_output() -> None:
    mock_apm = make_mock_apm()
    mock_apm.gws.workloads.list_versions.return_value = ([SAMPLE_VERSION], 1)
    ...
    assert data[0]["version_id"] == "ver-gws-001"

def test_gws_mail_version_list_csv_output() -> None:
    """gws mail version list --output csv should output flat CSV with location_count, not nested locations list."""
    mock_apm = make_mock_apm()
    mock_apm.gws.workloads.list_versions.return_value = ([SAMPLE_VERSION], 1)
...
def test_gws_mail_version_list_search_mode() -> None:
    """version list search mode resolves domain then workload."""
    mock_apm = make_mock_apm()
    mock_apm.gws.workloads.list_versions.return_value = ([], 0)

    result = invoke_cli(mock_apm, [
        "gws", "mail", "version", "list",
        "alice@gwsdemo.example.com", "-d", DOMAIN_ID,
    ])

    assert result.exit_code == 0, result.output
    mock_apm.gws.workloads.get_by_name.assert_called_once()
    mock_apm.gws.workloads.list_versions.assert_called_once()

def test_gws_mail_version_list_no_args_shows_help() -> None:
    result = invoke_cli(AsyncMock(), ["gws", "mail", "version", "list"])

    assert result.exit_code == 0
    assert "Usage" in result.output

def test_gws_mail_version_get_direct_mode() -> None:
    """version get --workload-id --namespace --id (direct mode)."""
    mock_apm = make_mock_apm()
    mock_apm.gws.workloads.get_version.return_value = SAMPLE_VERSION
    mock_apm.activities.backup.get_by_version.return_value = SAMPLE_ACT

    result = invoke_cli(mock_apm, [
        "gws", "mail", "version", "get",
        "--workload-id", WORKLOAD_ID, "--namespace", NAMESPACE,
        "--id", "ver-gws-001",
    ])
    ...
    mock_apm.gws.workloads.get.assert_called_once_with(
        WORKLOAD_ID, NAMESPACE, domain=DOMAIN_ID, workload_type=GWSWorkloadType.MAIL
    )
    mock_apm.gws.workloads.get_version.assert_called_once_with(SAMPLE_WL, "ver-gws-001")

def test_gws_mail_version_get_search_mode() -> None:
    ...
    result = invoke_cli(mock_apm, [
        "gws", "mail", "version", "get",
        "alice@gwsdemo.example.com", "-d", DOMAIN_ID, "--id", "ver-gws-001",
    ])
...
def test_gws_mail_version_get_json_output() -> None:
    mock_apm = make_mock_apm()
    mock_apm.gws.workloads.get_version.return_value = SAMPLE_VERSION
    mock_apm.activities.backup.get_by_version.return_value = SAMPLE_ACT
    ...
    result = invoke_cli(mock_apm, [
        "gws", "mail", "version", "get",
        "--workload-id", WORKLOAD_ID, "--namespace", NAMESPACE,
        "--id", "ver-gws-001", "--output", "json",
    ])

    assert result.exit_code == 0, result.output
    import json as _json
    data = _json.loads(result.stdout)
    assert data["version_id"] == "ver-gws-001"
    assert data["activity"]["activity_id"] == "act-gws-001"

def test_gws_mail_version_get_not_found_exits_1() -> None:
    """version get with non-existent version_id should exit 1."""
    from synology_apm.sdk.exceptions import ResourceNotFoundError
    mock_apm = make_mock_apm()
    mock_apm.gws.workloads.get_version.side_effect = ResourceNotFoundError(
...
def test_gws_mail_version_get_no_args_shows_help() -> None:
    result = invoke_cli(AsyncMock(), ["gws", "mail", "version", "get"])

    assert result.exit_code == 0
    assert "Usage" in result.output

def test_gws_mail_version_get_latest_search_mode() -> None:
    """version get without --id (search mode) should call get_latest_version()."""
    mock_apm = make_mock_apm()
    mock_apm.gws.workloads.get_latest_version.return_value = SAMPLE_VERSION
    mock_apm.activities.backup.get_by_version.return_value = SAMPLE_ACT
    ...
    assert result.exit_code == 0, result.output
    assert "(Using version: ver-gws-001, created at" in result.output
    mock_apm.gws.workloads.get_latest_version.assert_called_once()
    mock_apm.gws.workloads.list_versions.assert_not_called()

def test_gws_mail_version_get_latest_direct_mode() -> None:
    ...
    assert result.exit_code == 0, result.output
    assert "(Using version: ver-gws-001, created at" in result.output
    mock_apm.gws.workloads.get_latest_version.assert_called_once_with(SAMPLE_WL)
    mock_apm.gws.workloads.list_versions.assert_not_called()

def test_gws_version_get_shows_processed_items() -> None:
    """version get detail should display the Processed items row (GWS activity)."""
...
def test_gws_mail_version_lock_direct_mode() -> None:
    ...
    mock_apm.gws.workloads.get_version.assert_called_once_with(SAMPLE_WL, "ver-gws-001")
    mock_apm.gws.workloads.lock_version.assert_called_once_with(SAMPLE_VERSION)

def test_gws_mail_version_lock_search_mode() -> None:
...
def test_gws_mail_version_lock_no_id_shows_help() -> None:
    """version lock without --id should show help and exit 0."""
    result = invoke_cli(AsyncMock(), ["gws", "mail", "version", "lock", "alice@gwsdemo.example.com", "-d", DOMAIN_ID])

    assert result.exit_code == 0
    assert "Usage" in result.output

def test_gws_mail_version_unlock_direct_mode() -> None:
    """version unlock in direct mode should fetch workload, look up the version, then call unlock_version."""
    mock_apm = make_mock_apm()
    mock_apm.gws.workloads.get_version.return_value = SAMPLE_VERSION

    result = invoke_cli(mock_apm, [
        "gws", "mail", "version", "unlock",
        "--workload-id", WORKLOAD_ID, "--namespace", NAMESPACE,
        "--id", "ver-gws-001",
    ])
    ...
    mock_apm.gws.workloads.get_version.assert_called_once_with(SAMPLE_WL, "ver-gws-001")
    mock_apm.gws.workloads.unlock_version.assert_called_once_with(SAMPLE_VERSION)

def test_gws_mail_version_get_name_and_workload_id_conflict_exits_1() -> None:
    """version get NAME --workload-id X should print error and exit 1."""
    result = invoke_cli(AsyncMock(), [
        "gws", "mail", "version", "get",
        "alice@gwsdemo.example.com", "--workload-id", WORKLOAD_ID,
    ])

    assert result.exit_code == 1
    assert "cannot be used" in result.output

def test_gws_mail_version_get_workload_id_without_namespace_exits_1() -> None:
...
def test_gws_mail_version_get_shows_log_entries() -> None:
...
def test_gws_mail_version_lock_name_and_workload_id_conflict_exits_1() -> None:
    """version lock NAME --workload-id X should print error and exit 1."""
    result = invoke_cli(AsyncMock(), [
        "gws", "mail", "version", "lock",
        "alice@gwsdemo.example.com", "--workload-id", WORKLOAD_ID, "--id", "ver-gws-001",
    ])

    assert result.exit_code == 1
    assert "cannot be used" in result.output

def test_gws_mail_version_lock_workload_id_without_namespace_exits_1() -> None:
    """version lock --workload-id X --id V (no --namespace) should print error and exit 1."""
    result = invoke_cli(AsyncMock(), [
        "gws", "mail", "version", "lock",
        "--workload-id", WORKLOAD_ID, "--id", "ver-gws-001",
    ])
...
def test_gws_mail_version_get_lists_all_locations() -> None:
    ...
    mock_apm = make_mock_apm()
    mock_apm.gws.workloads.get_version.return_value = two_location_version
    mock_apm.activities.backup.get_by_version.return_value = SAMPLE_ACT
