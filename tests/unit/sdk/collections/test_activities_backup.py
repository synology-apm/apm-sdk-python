"""Unit tests for BackupActivityCollection."""
...
async def test_list_m365_types_sends_saas_service_type_params() -> None:
    ...
    async with connected_session() as (session, m):
        ...
        # aiohttp sorts params alphabetically: M365_TEAMS < M365_USER_EXCHANGE
        pat = re.compile(r".*/api/v2/activity/backup/activities\?.*saasServiceType=M365_TEAMS.*saasServiceType=M365_USER_EXCHANGE.*")
        m.get(pat, payload={"activities": [SAMPLE_M365_ACTIVITY_RUNNING]})
        acts, total = await BackupActivityCollection(session).list(
            m365_types=[M365WorkloadType.EXCHANGE, M365WorkloadType.TEAMS]
        )
        await session.disconnect()
