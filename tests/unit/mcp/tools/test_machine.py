...
class TestListMachineWorkloads:
    ...
    async def test_plan_ids_resolves_protection_plan_and_forwards_filter(self, mock_apm: MagicMock, mock_ctx: MagicMock) -> None:
        ...
        await call_tool(server, "list_machine_workloads", mock_ctx, plan_ids=["plan-001"])
    ...
    @pytest.mark.asyncio
    async def test_plan_ids_falls_back_to_retirement_plan_when_not_a_protection_plan(self, mock_apm: MagicMock, mock_ctx: MagicMock) -> None:
        from synology_apm.mcp._server import create_server
        ...
        server = create_server(mode="admin")
        await call_tool(server, "list_machine_workloads", mock_ctx, plan_ids=["ret-001"])
    ...
    @pytest.mark.asyncio
    async def test_hypervisor_id_forwarded_to_sdk(self, mock_apm: MagicMock, mock_ctx: MagicMock) -> None:
        from synology_apm.mcp._server import create_server

        mock_apm.machine.workloads.list.return_value = ([], 0)

        server = create_server(mode="admin")
        await call_tool(server, "list_machine_workloads", mock_ctx, hypervisor_id="hyp-001")
    ...
    async def test_plan_ids_and_hypervisor_id_default_to_none(self, mock_apm: MagicMock, mock_ctx: MagicMock) -> None:
        ...
        server = create_server(mode="admin")
        await call_tool(server, "list_machine_workloads", mock_ctx)
    ...
    async def test_status_forwarded_to_sdk_as_enum_list(self, mock_apm: MagicMock, mock_ctx: MagicMock) -> None:
        ...
        server = create_server(mode="admin")
        await call_tool(server, "list_machine_workloads", mock_ctx, status=["failed", "partial"])

        _, kwargs = mock_apm.machine.workloads.list.call_args
        assert kwargs["status"] == [WorkloadStatus.FAILED, WorkloadStatus.PARTIAL]

    @pytest.mark.asyncio
    async def test_verify_status_forwarded_to_sdk_as_enum_list(self, mock_apm: MagicMock, mock_ctx: MagicMock) -> None:
        from synology_apm.mcp._server import create_server
        ...
        server = create_server(mode="admin")
        await call_tool(server, "list_machine_workloads", mock_ctx, verify_status=["not_enabled"])

        _, kwargs = mock_apm.machine.workloads.list.call_args
        assert kwargs["verify_status"] == [VerifyStatus.NOT_ENABLED]

    @pytest.mark.asyncio
    async def test_status_and_verify_status_default_to_none(self, mock_apm: MagicMock, mock_ctx: MagicMock) -> None:
        from synology_apm.mcp._server import create_server
...
class TestBackupMachineWorkload:
    @pytest.mark.asyncio
    async def test_calls_backup_now(self, mock_apm: MagicMock, mock_ctx: MagicMock, admin_server: FastMCP) -> None:
        wl = make_machine_workload()
    ...
    async def test_unknown_workload_returns_structured_error(self, mock_apm: MagicMock, mock_ctx: MagicMock, tmp_path: Path) -> None:
        ...
        with patch.dict(os.environ, {"APM_MCP_AUDIT_LOG": str(log_file)}), pytest.raises(ToolError) as exc_info:
            await call_tool(server, "backup_machine_workload", mock_ctx, workload_id="does-not-exist", namespace="default")
...
class TestGetMachineWorkload:
    ...
    async def test_resolves_by_id_and_namespace(self, mock_apm: MagicMock, mock_ctx: MagicMock) -> None:
        ...
        server = create_server(mode="admin")
        result = await call_tool(server, "get_machine_workload", mock_ctx, workload_id=_WL_ID, namespace="default")

        mock_apm.machine.workloads.get.assert_called_once_with(_WL_ID, "default")
        assert result["is_retired"] is True

    @pytest.mark.asyncio
    async def test_not_found_returns_structured_error(self, mock_apm: MagicMock, mock_ctx: MagicMock, admin_server: FastMCP) -> None:
        from synology_apm.sdk import ResourceNotFoundError
        ...
        with pytest.raises(ToolError) as exc_info:
            await call_tool(admin_server, "get_machine_workload", mock_ctx, workload_id=_WL_ID, namespace="default")
...
class TestGetMachineVerificationVideoUrl:
    @pytest.mark.asyncio
    async def test_returns_url_for_resolved_version(self, mock_apm: MagicMock, mock_ctx: MagicMock, admin_server: FastMCP) -> None:
        wl = make_machine_workload()
...
class TestUpdateMachineFileServer:
    @pytest.mark.asyncio
    async def test_sends_only_explicitly_supplied_fields_no_merge(self, mock_apm: MagicMock, mock_ctx: MagicMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_audit_log_records_workload_id_and_host_ip(self, mock_apm: MagicMock, mock_ctx: MagicMock, tmp_path: Path) -> None:
        import os
        from unittest.mock import patch
    ...
    @pytest.mark.asyncio
    async def test_login_password_none_is_a_valid_explicit_keep_current_value(self, mock_apm: MagicMock, mock_ctx: MagicMock) -> None:
        from synology_apm.mcp._server import create_server
    ...
    async def test_missing_required_field_raises(self, mock_ctx: MagicMock) -> None:
        ...
        with pytest.raises(TypeError):
            await call_tool(server, "update_machine_file_server", mock_ctx, workload_id=_WL_ID, namespace="default", host_ip="192.0.2.1", login_user="admin")

    @pytest.mark.asyncio
    async def test_selectors_param_builds_multi_selector_request_with_excluded_paths(self, mock_apm: MagicMock, mock_ctx: MagicMock) -> None:
        from synology_apm.mcp._server import create_server
    ...
    @pytest.mark.asyncio
    async def test_raises_when_neither_path_nor_selectors_supplied(self, mock_apm: MagicMock, mock_ctx: MagicMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_raises_when_both_path_and_selectors_supplied(self, mock_apm: MagicMock, mock_ctx: MagicMock) -> None:
        from synology_apm.mcp._server import create_server
...
class TestAddMachineFileServer:
    @pytest.mark.asyncio
    async def test_defaults_to_single_root_selector_when_path_and_selectors_omitted(self, mock_apm: MagicMock, mock_ctx: MagicMock) -> None:
        from synology_apm.mcp._server import create_server
        from synology_apm.sdk import FileServerPathSelector

        server = create_server(mode="admin")
        await call_tool(
            server, "add_machine_file_server", mock_ctx,
            namespace="default",
            host_ip="192.0.2.1",
            server_type="smb",
            plan_id="plan-001",
    ...
    @pytest.mark.asyncio
    async def test_uses_selectors_param_for_multiple_paths_with_exclusions(self, mock_apm: MagicMock, mock_ctx: MagicMock) -> None:
        from synology_apm.mcp._server import create_server
        from synology_apm.sdk import FileServerPathSelector
