...
class TestListBackupActivities:
    ...
    @pytest.mark.asyncio
    async def test_namespaces_forwarded_as_list(self, mock_apm: MagicMock, mock_ctx: MagicMock) -> None:
        from synology_apm.mcp._server import create_server

        mock_apm.activities.backup.list.return_value = ([], 0)

        server = create_server(mode="admin")
        await call_tool(server, "list_backup_activities", mock_ctx, namespaces=["default", "secondary"])
    ...
    @pytest.mark.asyncio
    async def test_machine_types_and_m365_types_mutually_exclusive(self, mock_apm: MagicMock, mock_ctx: MagicMock) -> None:
        from synology_apm.mcp._server import create_server
    ...
    async def test_machine_workload_scoping_resolves_and_forwards_workload(self, mock_apm: MagicMock, mock_ctx: MagicMock) -> None:
        ...
        server = create_server(mode="admin")
        await call_tool(server, "list_backup_activities", mock_ctx, workload_id=_WL_ID, workload_namespace="default")
    ...
    @pytest.mark.asyncio
    async def test_m365_workload_scoping_requires_workload_type(self, mock_ctx: MagicMock) -> None:
        from synology_apm.mcp._server import create_server

        server = create_server(mode="admin")
        with pytest.raises(ValueError, match="workload_type is required"):
            await call_tool(server, "list_backup_activities", mock_ctx, workload_id="wl-001", workload_namespace="default", tenant_id="tenant-001")

    @pytest.mark.asyncio
    async def test_workload_id_without_namespace_raises(self, mock_ctx: MagicMock) -> None:
        from synology_apm.mcp._server import create_server

        server = create_server(mode="admin")
        with pytest.raises(ValueError, match="workload_namespace is required"):
            await call_tool(server, "list_backup_activities", mock_ctx, workload_id="wl-001")

    @pytest.mark.asyncio
    async def test_m365_workload_scoping_resolves_and_forwards_workload(self, mock_apm: MagicMock, mock_ctx: MagicMock) -> None:
        from synology_apm.mcp._server import create_server
        ...
        server = create_server(mode="admin")
        await call_tool(
            server, "list_backup_activities", mock_ctx,
...
class TestGetBackupActivity:
    @pytest.mark.asyncio
    async def test_returns_activity_dict(self, mock_apm: MagicMock, mock_ctx: MagicMock, admin_server: FastMCP) -> None:
        from tests.unit.mcp.conftest import call_tool

        act = make_backup_activity()
        mock_apm.activities.backup.get.return_value = act

        result = await call_tool(admin_server, "get_backup_activity", mock_ctx, activity_id="act-001")
...
class TestCancelBackupActivity:
    @pytest.mark.asyncio
    async def test_calls_cancel_and_returns_ok(self, mock_apm: MagicMock, mock_ctx: MagicMock, admin_server: FastMCP) -> None:
        from tests.unit.mcp.conftest import call_tool
        ...
        result = await call_tool(admin_server, "cancel_backup_activity", mock_ctx, activity_id="act-001")
...
class TestListRestoreActivities:
    ...
    @pytest.mark.asyncio
    async def test_workload_scoping_resolves_and_forwards_workload(self, mock_apm: MagicMock, mock_ctx: MagicMock) -> None:
        from synology_apm.mcp._server import create_server
        ...
        server = create_server(mode="admin")
        await call_tool(server, "list_restore_activities", mock_ctx, workload_id=_WL_ID, workload_namespace="default")
    ...
    @pytest.mark.asyncio
    async def test_workload_scoping_omitted_forwards_none(self, mock_apm: MagicMock, mock_ctx: MagicMock) -> None:
        from synology_apm.mcp._server import create_server

        mock_apm.activities.restore.list.return_value = ([], 0)

        server = create_server(mode="admin")
        await call_tool(server, "list_restore_activities", mock_ctx)
...
class TestGetRestoreActivity:
    ...
    async def test_returns_activity_dict(self, mock_apm: MagicMock, mock_ctx: MagicMock, admin_server: FastMCP) -> None:
        ...
        result = await call_tool(admin_server, "get_restore_activity", mock_ctx, activity_id="rst-001")
...
class TestCancelRestoreActivity:
    ...
    async def test_calls_cancel_and_returns_ok(self, mock_apm: MagicMock, mock_ctx: MagicMock, admin_server: FastMCP) -> None:
        ...
        result = await call_tool(admin_server, "cancel_restore_activity", mock_ctx, activity_id="rst-001")
        assert result["ok"] is True
        mock_apm.activities.restore.get.assert_called_once_with("rst-001")
...
class TestLimitValidation:
    @pytest.mark.asyncio
    async def test_large_limit_is_accepted_and_forwarded(self, mock_apm: MagicMock, mock_ctx: MagicMock) -> None:
        """A large limit is not rejected (no upper bound) and is forwarded to the SDK
        call unchanged -- exercised through the real tool body via call_tool(), since
        real dispatch through admin_server would need a live APM connection to reach
        the tool body once validation passes."""
        from synology_apm.mcp._server import create_server

        mock_apm.activities.backup.list.return_value = ([], 0)

        server = create_server(mode="admin")
        await call_tool(server, "list_backup_activities", mock_ctx, limit=10_000)
