...
- At least one backup server (`synology-apm infra server list`).
- At least one **DP-type** appliance (`synology-apm infra server list --type dp`) —
...
- At least one active Machine Workload (`synology-apm machine list`).
- At least one of them has a backup version
  (`synology-apm machine version list <name>`) — needed to exercise `version get`. The
...
- Optional: at least one **retired** Machine Workload
  (`synology-apm machine list --retired`) — exercises a non-empty `list --retired`.

## M365 / SaaS

- At least one M365-category tenant (`synology-apm saas list`) — **required**: without it,
  the entire `m365` domain is skipped.
- For each of the six scopes — `exchange`, `onedrive`, `chat`, `group`, `sharepoint`,
  `teams` — at least one workload (`synology-apm m365 <scope> list`) with at least one backup
  version (`synology-apm m365 <scope> version list <name>`). The phase prefers a workload
...
- Optional: at least one **retired** workload in any scope (`synology-apm m365 <scope> list
  --retired`) — exercises a non-empty `list --retired` for that scope.

## Activity

- Backup activity history (`synology-apm activity backup list --history`) — populated
...
- Optional: restore activity history (`synology-apm activity restore list --history`), to
  exercise the `activity restore list --search` / `get` steps.

## Plan

- At least one Protection Plan and one Retirement Plan
  (`synology-apm plan protection list` / `synology-apm plan retirement list`).
- Optional: at least one Tiering Plan (`synology-apm plan tiering list`).
